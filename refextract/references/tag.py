...
def process_reference_line(working_line,
    ...
    if len(journals_matches) + len(pprint_repnum_len) + len(publishers_matches) == 0:
        ...
        replacement_types = {}
        journals_keys = list(journals_matches.keys())
        journals_keys.sort()
        reports_keys = list(pprint_repnum_matchtext.keys())
        reports_keys.sort()
        publishers_keys = list(publishers_matches.keys())
        publishers_keys.sort()
        spaces_keys = list(removed_spaces.keys())
        spaces_keys.sort()
        replacement_types = get_replacement_types(journals_keys,
                                                  reports_keys,
                                                  publishers_keys)
        replacement_locations = list(replacement_types.keys())
...
def identify_report_numbers(line, kb_reports):
    ...
    repnum_categs = list(repnum_standardised_categs.keys())
    repnum_categs.sort(key=cmp_to_key(_by_len))
...
def identify_and_tag_URLs(line):
    ...
    found_url_positions = list(found_url_urlstring.keys())
